...
- **观测遗留（真缺陷，非阻塞，已定位不在注册层）**：`dw.readiness.*` 六指标**两轮真跑均不在 `/actuator/prometheus` 出现**（含 Gauge，本应恒可见）。**上轮"测量假象"判定已被推翻**。probe 单测 `SchedulerMetricsTest.readiness六指标全部注册到registry_对照dispatch` 证明：**注册代码正确**——6 指标连同 dispatch/claim 全部进 MeterRegistry。且无 MeterFilter/`metrics.enable`/distributed 专属 yml 过滤。故缺陷**隔离在 Prometheus 导出/scrape 层**（注册成功但活体 `/actuator/prometheus` 未渲染 `dw_readiness_*`），非 SchedulerMetrics 代码。**下次真跑决定性诊断**：`curl /actuator/metrics | grep readiness`（**LIST 端点，点名**）—— ① 若出现而 `/actuator/prometheus` 无 → Prometheus 渲染器问题（histogram/命名）；② 若两端点皆无 → SchedulerMetrics 注入的 registry ≠ actuator 的 Prometheus registry（bean 装配/时序）。同时确认同一 scrape 里 `dw_dispatch_*` 是否在（区分"仅 readiness 缺"还是"全 dw.* 缺"）。**不影响正确性与 materialized 门控**（后者依据 T026 no-loss SQL 证据，与指标暴露正交；就绪滞后 p99 功能上由 T026 信号计数间接佐证）。
