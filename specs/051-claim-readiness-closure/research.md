...
- **T029 slot_util ✅ 达标（2026-07-06 复测，含 gauge 采样器修复镜像）**：稳态 **slot_utilization ≥80% 达成**——三通道一致：Prometheus `scheduler_slot_utilization`=1.0 + `/api/ops/metrics`=1.0 + DB 实况 16~23/20（20 slot 满载）。① slot 成绑定约束成立（453~889 WAITING 积压、RUNNING 顶 20 上限、emptyClaimRounds=5661、瓶颈转 worker）；③ FAILED 归零（28 FAILED 全为 WORKER_RESTART/OOM，**0 个 request timed out**，上轮 35 超时经调高回调超时消除）；gauge 采样器修复实证生效（无负载 0.0→有负载 1.0 动态）。**SC-003/007 闭合**。压测档：120 慢任务 `sleep10 */5s` + 快任务混合。
- **T024 SC-001/002 ✅ 达标（另一 agent 真跑）**：DB 直接注入 WAITING（绕 cron 生成瓶颈，压测标准做法）——**2000× 积压增长（246→500,498 WAITING）→ avg round_duration 仅 179.2ms→179.9ms（+0.4%）**，`dw_claim_window_extra_count_total=0`（无饥饿缓解触发）、50 万+ 实例下 master+2worker 无 OOM/宕机。**SC-001（claim 吞吐不随积压退化）+ SC-002（round_duration 与 WAITING 脱钩）闭合**——就绪判定 O(1)、与 WAITING 规模脱钩的结构解核心命题实证成立。（口径备注：注入实例部分 unmet_deps 可能非零，实际被索引 seek 扫描行数 ≤ ready 子集；但 `WHERE unmet_deps=0 ... LIMIT n` 本就是索引 range scan O(log N+n)，与总 WAITING 脱钩的结论不依赖注入行的就绪比例。）
- **F4 观测层缺陷 —— 已证非 SchedulerMetrics 代码根因**：`dw.readiness.*` 六指标两轮真跑不在 `/actuator/prometheus` 出现。**两级证据**：① probe 证注册正确；② **决定性** `ReadinessMetricsPrometheusExportTest`（真 `io.micrometer.prometheusmetrics.PrometheusMeterRegistry`，镜像 actuator 导出路径）——`scrape()` 输出六指标全部出现（`Tests run:1, Failures:0`）。故**导出代码正确并加回归守卫**；F4 残余隔离在**活体 actuator/Spring 装配或测量口径**，非代码、非门控（与 T026 no-loss SQL 证据正交）。剩余定位需活集群 1-2 行 curl（见 audit §3-F4），且定位后均非改 SchedulerMetrics。
