...
- **✅ T041 证伪式真跑门禁（docker distributed 2 master+2 worker，2026-07-10 实测）**：起 PG+redis+neo4j+2master+2worker，PG 重建到 schema 0.15.0（旧卷 0.14.3 缺 060 列，`SPRING_SQL_INIT_MODE=never` 不迁移——**这本身验证了 060 schema 升级在真集群生效**）。实测证据链：① 真实 cron 触发实例 `019f49ce…` 下发到**真实 worker-1、attempt=1、SUCCESS**（CLAUDE.md 下发门：root attempt=1）；② 只路由到心跳新鲜的真实 worker，**陈旧种子节点 node-1..5 被 060 SlotManager 门排除**（原幽灵节点 bug 结构性修复）；③ 真实 worker 注册后 `incarnation_since`/`consecutive_infra_failures`/`quarantined_until` 全部正确填充；④ **kill worker-1 → 经 060 `markStaleOffline`(@Scheduled) 判 OFFLINE，worker-2 存活，两 master 不崩**；⑤ **重启 worker-1 → 新 incarnation → FleetService 实时日志「节点 worker-1 incarnation 变化 1783649732→1783650222，worker 重启」→ I1 节点级重启处理触发**，回归 ONLINE；⑥ **schema 重载后两 master 稳态零 ERROR**（此前报错全为重载前旧 schema 残留）。其余 SC（SC-003 无节点等待/SC-006 分区自我中止/SC-007 Flink 集群 reattach/SC-008 毒任务 SUSPENDED/SC-009 人工兜底）由 152+ 确定性单测/集成覆盖；本次真跑确证 schema 部署 + 下发门 + 节点剔除 + 重启纪元四条核心链路在真集群成立。
