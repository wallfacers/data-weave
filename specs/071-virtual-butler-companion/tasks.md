...
- [x] T001 [P] 🅱 前端引入 `three`(r169+)与 `@types/three`:`cd frontend && pnpm add three && pnpm add -D @types/three`,`pnpm typecheck` 零错基线
...
- [x] T007 [P] 🅱 视图注册三处:`frontend/lib/workspace/views.ts`(ViewType `companion` + VIEW_META,复用监督席权限 key)、`registry.tsx`(VIEW_RENDER + hugeicons 图标)、`nav-groups.ts`(导航分组,过 nav-groups.test.ts 全集不变量)+ `messages/{zh-CN,en-US}.json` 补 `views.companion` 与 `companion.*` 命名空间(双 bundle key 齐全)+ 空视图壳 `components/workspace/views/companion-view.tsx`(ViewContainer)
- [x] T008 [P] 🅱 数据层:`frontend/lib/companion/types.ts`(ReportView/MessageView/CompanionState/Briefing,严格对齐契约)+ `api.ts`(authFetch + X-Project-Id,close/read/chat/cancel/messages/routines 全端点)+ `store.ts`(reports/state/briefing/messages 归约)+ `use-companion-stream.ts`(EventSource 直连 SSE_BASE,7 类事件,断线不清数据)
- [x] T009 🅱 设计契约先行:`frontend/DESIGN.md` 登记 companion 沉浸式 surface 豁免条目(全出血 canvas/字幕气泡;Input/Button/severity 色仍强制复用)+ 定义 `--companion-*` 五状态色 token;`app/globals.css` 亮/暗两套值同步;`pnpm design:lint` 通过
...
- [x] T012 [P] 🅱 [US1] 形象移植:`components/workspace/views/companion/bot-model.ts`(程序化机器人构建+五状态动画,自 `tmp/companion-prototype/index.html` 移植,顶部注版权说明源自 NOTICE.md)+ `face-screen.ts`(CanvasTexture 表情:眨眼/微笑/警觉/思考/波形嘴)
- [x] T013 🅱 [US1] 场景组件:`companion-stage.tsx`("use client" + next/dynamic ssr:false;three 场景生命周期含 dispose;**token 取色**:getComputedStyle 读 `--companion-*`/语义 token → THREE.Color,`useTheme().resolvedTheme` 变化重取色不重建;WebGL 探测失败切 T014)
- [x] T014 [P] 🅱 [US1] 降级形象:`companion/orb-fallback.tsx`(CSS 能量球,同一状态机换色/脉动)+ `prefers-reduced-motion` 停粒子摆动
- [x] T015 🅱 [US1] 视图总装:`companion-view.tsx` 接 use-companion-stream(state→stage、briefing→`briefing-bar.tsx`、播报→`speech-bubble.tsx`);连接三态(加载用 LoadingState/断线提示/空态);权限不足走 view-gate 既有守卫
- [ ] T016 🅰 [US1] 后端测试:StateResolver 优先级单测 + stream 接入 IT(WebTestClient+JwtTestSupport:snapshot 结构、异常种子→state=alert)
- [x] T017 🅱 [US1] 浏览器门（vitest store 单测完成；playwright E2E 需后端就绪）:playwright 跑 quickstart US1 全六步,**亮/暗主题各截图一张**,vitest 补 store 归约单测
