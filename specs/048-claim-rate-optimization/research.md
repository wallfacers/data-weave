...
## R10. 实测结论(2026-07-06,distributed 双 master 1000wf */2s + 048 批量化)

**048 实施范围**:T003(Row+workflowId)+ T006(batchCrossCycleReady 批量 JOIN+COUNT)+ T007(claimAndMark filter)+ T008(casDispatchBatch UPDATE FROM VALUES)+ T009(assign 三阶段)。单测 14 绿(InstanceStateMachineTest 4 + SchedulerKernelBatchCrossCycleTest 10)。commit 637503b 核心 + a70ec8e 单测。

**部分收益(048 批量化早期生效)**:
- round_duration 早期 **48.9ms**(046 R9 ~100-150ms,**~2-3x 提升**)
- dispatch_latency avg **6.1ms**(282.7s/46275,vs 046 R9 135.7s,被 claim 的实例极快)
- dispatch_queue.size 2-18(046 成果不退化),queue.full=0

**SC-002 未达成(WAITING 堆积)**:
- WAITING 持续涨(181366 → 534698),claim/dispatch 仅 **13 inst/s** << 物化 ~1000 inst/s(2000wf */2s 含 046 残留)
- round_duration 随 WAITING 堆积**反弹**:早期 49ms → 全程 avg 128ms → 近期 1.58s/round(max 2.81s)
- rounds/s 仅 0.7-1.4(应 ~20)

**根因诊断(selectRunnable 重 SQL 在大表退化,非 048 批量化范围)**:
- 048 批量化只降 crossCycleReady/casDispatch,**未动 selectRunnable**
- selectRunnable NORMAL SQL(NOT EXISTS 上游门 + 多标量子查询 task_def/workflow_instance)在 WAITING 53 万行大表上退化(简化版已 61ms/50 行,完整版 1s+)
- idx_task_instance_claim(state,run_mode,deleted,updated_at)已存在,但 NOT EXISTS 子查询 + 标量子查询不走索引
- worker rebuild 排除 worker 干扰:dispatch 速率不变(13 inst/s),slot_util=0 是 ECHO 秒完成统计窗口(DISPATCHED=0,SUCCESS 持续涨)

**SC 达成**:
- SC-003(幂等无重复)✓(casDispatchBatch WHERE state='WAITING' CAS,updateCount 核对)
- SC-001(dispatch_latency<5s):**被 claim 的** 6.1ms ✓,但 WAITING 排队(未 claim 的)堆积 → 端到端未达成
- SC-002(吞吐≥600 inst/s)✗(claim 13 inst/s)

**结论**:048 批量化核心正确(round_duration 早期 3x + dispatch_latency 6.1ms),但 **selectRunnable 重 SQL 是更深瓶颈**(大表退化,048 未覆盖)。后续 049 方向:
1. selectRunnable 优化:JOIN 去标量子查询 + NOT EXISTS 上游门简化
2. 防 WAITING 堆积(claim 跟上物化的根本解)
3. worker rebuild 已验证非瓶颈
