...
- Q: 旧 `create_task` 工具(MCP 里内联 SQL 直建单任务+上线)何去何从? → A: **移除**。任务即代码下创建/修改定义一律走 `project_push`(文件化);血缘 io 声明(原 `@io` 头)改由 B 文件契约承载,不丢能力。
- Q: `project_push` 写闸门风险等级怎么定? → A: **风险自适应** —— 纯新增/更新 → L1 直通+审计;含删除或 `force` 覆盖 → 升 L2 审批挂起。由 `policy_rules` 表驱动(seed 一条 PROJECT_PUSH 规则),与 C 的删除守卫语义同调。
- Q: 复用的只读工具当前 `findAll()` 跨租户(既有越权读漏洞),E 是否补隔离? → A: **补**。所有 MCP 工具(含既有 query_*)一律按 `TenantContext` 过滤,修掉既有跨租户读漏洞。
...
- **FR-002a**: 旧 `create_task` 工具(内联 SQL 直建单任务+上线)MUST 被移除;经 MCP 的定义创建/修改 MUST 一律走 `project_push`(文件化),不保留绕过文件表示的内联写入路径(贯彻原则 I 文件优先)。原 `@io` 设计态血缘声明改由 B 文件契约承载。
...
- **FR-006**: 触达 L2/L3 风险等级的写操作 MUST 进入审批挂起(PENDING_APPROVAL),在人工批准前 MUST NOT 落库/执行;L4 MUST 拒绝;L0/L1 直接执行。`project_push` 的等级 MUST **风险自适应**:纯新增/更新 → L1 直通+审计;含删除或 `force` 覆盖 → L2 审批挂起;经 `policy_rules` 表驱动(seed PROJECT_PUSH 规则),与 C 删除守卫语义同调。整次 push 是**一个**闸门决策(对齐 C 的 @Transactional 全有或全无):挂起则不执行 push,通过才调 `ProjectSyncService.push`。
- **FR-007**: 所有 MCP 工具(读与写)MUST 受租户/项目隔离约束;越权 MUST 被拒,无跨租户数据泄漏。E MUST 修复既有只读工具的隔离缺口 —— 当前 `query_*` 用 `taskDefRepository.findAll()` 等**跨租户**返回,MUST 改为按 `TenantContext` 过滤。
...
- **SC-001**: `tools/list` 返回的工具 100% 属于重塑后清单,0 个已拆除 AI 专用工具残留,0 个 create_task 类内联写入工具。
- **SC-002**: 经 MCP 的写操作 100% 留下 agent_action 审计,0 旁路;构造的含删除/force 的 project_push 100% 进 PENDING_APPROVAL 且批准前 0 落库;纯新增/更新 push 100% L1 直通+审计。
- **SC-003**: 越权/跨租户 MCP 调用(含既有 query_* 工具)100% 被拒,0 数据泄漏。
...
- 工具集边界已部分裁定(见 Clarifications):**移除 create_task**、**新增 project_pull/push/diff**、**所有 query_* 补租户隔离**;其余保留工具(query_task_instances/fleet/metric/lineage、task_rerun、node_exec、approve_and_execute)的逐一去留在 plan 依 `McpToolRegistry` 实况定,但守"至少覆盖 + 写必过闸门 + 无残留 AI 工具 + 全隔离"。
- `project_push` 风险等级经 `policy_rules` 表数据驱动(seed PROJECT_PUSH 规则,默认风险自适应),plan 须给出 seed 与等级判定逻辑(删除/force → L2)。
- 只读隔离回补会触及既有 query_* 工具实现(把 findAll 改为按 TenantContext 过滤),属 E 必做范围,非新工具专属。
