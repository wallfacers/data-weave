...
| 合并策略（B2 残差，canon） | prec(全) | recall(非空) | 方向(非空) | f1(非空) | 判定 |
|---|---|---|---|---|---|
| model 独跑 | 0.352 | 0.727 | 0.560 | 0.642 | 基准 |
| override（SQL 整条覆盖） | 0.332 | 0.570 | 0.504 | 0.568 | ❌ 四项全低于基准（低召回 SQL 覆盖丢掉好模型的抽取） |
| union（读写并集，SQL 方向优先） | 0.305 | 0.727 | **0.610** | 0.612 | 〜 方向最高但 prec 塌（过度并入） |
| **dir_fix（模型定表 / AST 定方向）** | **0.358** | 0.697 | **0.596** | **0.648** | ✅ 方向 +3.6pt、f1 最优、无 trade-off |

（✅/❌/〜 均相对「model 独跑」基准而言：❌=全面倒退、〜=有得有失、✅=帕累托改进。）
