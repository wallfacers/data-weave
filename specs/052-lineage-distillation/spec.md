...
- **学生略逊 teacher**：蒸馏常态；若落到 recall~0.75（低于门但远高于现状 0.62）→ 触发升级路径并与维护者拍板是否分阶段上线。
...
- **FR-004**: 系统 MUST 构建银标：仅保留 m1∩m2 一致的表；teacher 提出的表名必须在脚本文本中字面出现才留（字面子串硬门滤幻觉）；方向优先取 AST 解析结果，不可解析时取 teacher 方向。
...
- **FR-007**: 系统 MUST 保留独立测试集：测试集 A（现有 139+141 金标）+ 测试集 B（新采、teacher 从未参与训练打标、经人工证伪裁决的新鲜集）。
...
- **FR-016**: 系统 MUST 以测试集 B 通过全部验收门为唯一"生产可用"判据；合成分数不作判据。
...
验收门锚定大模型 m2 基线（real.jsonl：recall 0.806 / 方向 0.730 / 幻觉 0.134 / precision 0.542），在 **held-out 真实测试集 B** 上全自动评测：
