...
**前置**：开发者本地提供真本地 agent（workhorse + 真实 LLM 配置：key/endpoint）。

**步骤**：
1. 在 Weft 仓库工作副本中起本地 agent（加载项目级 Skill）。
2. 对 agent 说"创建一个新任务 …"（自然语言诉求）。
3. 观察：agent 自动加载 `weft-task-authoring` Skill（无需手工粘说明），据其写出结构合法的任务文件，`dw run` 本地跑通，`dw diff` → `dw push` 上线。
4. agent 正确读 GateResult 三态（含删除/高危 push → 挂起待审批，不误判为失败）。
