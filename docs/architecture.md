...
## 3. 前端架构（Agent-Native 双栏，agent-workspace-m2）

- **双栏外壳**：左 = Agent 对话主驾（常驻、可拖拽调宽，`agent-rail.tsx` 内嵌 `AgentChat`），右 = **Workspace 多 tab 工作区**（`components/workspace/`）。根路由 `/` 即 Workspace；旧模块路由保留为 `/?open=<view>` 重定向深链。
- **对话**：`CopilotKitProvider selfManagedAgents={{ dataweave: httpAgent }}` + `<CopilotChat agentId="dataweave" />`，`HttpAgent({ url: NEXT_PUBLIC_AGENT_URL, threadId })` 直连后端；threadId 持久化（localStorage），与 Workspace 快照同会话 key。
- **Workspace 两层注意力模型**：Pinned 底座（驾驶舱/任务流/数据新鲜度/业务报表，恒定常驻）+ Ephemeral 召唤层（AI 经 `CUSTOM(dataweave.ui.open)` 或用户经 "+" 启动菜单打开，可关可 pin）。真相源 = 前端 zustand store（`lib/workspace/store.ts`），视图注册表在 `lib/workspace/registry.tsx`。
- **会话持久化**：store 变更防抖 PUT `/api/agent/sessions/{conversationId}/workspace`（快照只含 Ephemeral 与激活态），挂载时 GET 恢复；快照损坏/缺失回落 Pinned 底座。
- **双向上下文**：Workspace 激活 tab（view/params）经 `properties.dataweave` → 后端 forwardedProps，支撑「就当前所见」提问。
...
浏览器 (/)
  ...
  │                            └─ CUSTOM(dataweave.ui.open) ──▶ workspace store
  └─ Workspace（右侧多 tab：Pinned 底座 + Ephemeral 召唤层）
       └─ 快照 ⇄ GET/PUT /api/agent/sessions/{conversationId}/workspace
```
...
1. **Agent 对话界面** —— `/` 左栏主驾可输入中文、流式收 AG-UI 事件、渲染表格，并经 `ui.open` 召唤 Workspace 视图。
...
- **前端**：`cd frontend && pnpm dev`，打开 `http://localhost:4000`（左对话主驾 + 右 Workspace）。
...
DataWeave 支持 **zh-CN / en-US** 双语，采用 **混合归属策略**：两端各本地化自己擅长的文案，避免纯前端无法覆盖后端动态文本、纯后端 UI 控件啰嗦的两难。
...
| ② 后端动态生成 | AG-UI markdown、MCP 工具描述、诊断建议、审批理由 | 后端 `Messages.get` | **agent locale** | MessageFormat `{0}` |
...
- locale 协商：`Locales.uiLocale(headers)` 读 `Accept-Language`；agent 场景 `Locales.agentLocale(headers)` 读请求头 `x-dw-agent-locale`（AG-UI SSE 建连时一次传入，整条 run 沿用）。
