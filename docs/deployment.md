...
Weft 是前后端分离 + SSE 实时流的架构。生产上线时，**SSE（实时日志、工作流状态）
的流式行为对反向代理的缓冲配置敏感**——配错会表现为「日志一把输出而非滚屏」。本文给出
上线必改清单。

## SSE 防缓冲（重点）

实时日志（`/api/ops/.../logs/stream`）、工作流状态流（`/api/ops/workflow-instances/.../events/stream`）
都是 `text/event-stream` 长连接。任何中间代理若**缓冲**这些响应，就会把逐行 SSE
攒到上游关流才一次性下发，破坏实时性。
...
前端 `https://app.example.com`，后端 `https://api.example.com`。前端 SSE 直连后端域名。

需要配置：
- 前端构建期 env：`NEXT_PUBLIC_API_URL=https://api.example.com`
...
`https://app.example.com` 下，nginx 把 `/api` 转发后端。此时 SSE 经 nginx，
**nginx 默认 `proxy_buffering on` 会缓冲 SSE**，必须显式关闭：

```nginx
# SSE 流式端点：关闭缓冲、放宽超时
location ~ ^/api/ops/.+/(logs|events)/stream$ {
    proxy_pass http://weft_backend;
...
同源形态下前端可不设 `NEXT_PUBLIC_API_URL`（走同源 + nginx），但需确保 `SSE_BASE`
解析到同源后端入口；CORS 同源时不触发，无需放行额外源。

## 上线检查清单

- [ ] 后端 `app.cors.allowed-origins` 设为生产前端域名（形态 A 必须）
- [ ] 前端 `NEXT_PUBLIC_API_URL` 指向浏览器可达的后端地址
