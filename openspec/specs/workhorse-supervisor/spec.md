# workhorse-supervisor

## Purpose

托管 workhorse-agent sidecar 进程的生命周期：在 `agent.mode=workhorse` 下可选地由 DataWeave 后端拉起、adopt、回收并监控 sidecar 进程，提供 adopt-or-spawn、端口单实例互斥、崩溃退避重启、跨平台二进制选型、native/wsl 运行时与状态可观测能力。默认关闭，开启后行为对既有「外部托管 sidecar」完全向后兼容。
