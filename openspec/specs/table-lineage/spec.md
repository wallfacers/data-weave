# table-lineage Specification

## Purpose

定义 DataWeave 表级血缘能力：以「表为节点、任务为边」的二部图记录跨数据源血缘，建任务即生成设计态血缘边（三来源 AGENT/SQL_PARSED/FORM + 可信度 CONFIRMED/UNVERIFIED/CONFLICT），运行态采集真实同步行数，并支持上下游双向查询与 N 跳邻域子图拉取。
