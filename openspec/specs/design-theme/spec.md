...
Monaco 编辑器与 chat 代码块 SHALL 继续共用 `lib/syntax-palette.ts` 的同一套 `buildSyntaxTheme()` 主题对象；调色板 SHALL 从茶墨主题派生：keyword=茶墨家族、func=钴蓝、comment/operator=暖灰阶（完整角色表见 `frontend/lib/syntax-palette.ts`），亮/暗各一套。
