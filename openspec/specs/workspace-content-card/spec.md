# workspace-content-card Specification

## Purpose

定义 Workspace tab 条下方视图内容容器的表面规格：将内容区渲染为一张与左侧 Agent 对话面板同材质的浮起卡（`bg-sidebar` 底 + 边框 + 阴影 + 圆角），使白数据卡浮于卡面之上、左右并列读成一对抬升卡，且不引入冗余内层表面层。
