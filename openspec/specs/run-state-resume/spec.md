# run-state-resume Specification

## Purpose

定义数据开发任务编辑器与工作流画布「运行」入口的**单按钮 Run⇄Stop 状态机**及其运行态续接能力:运行按钮的显示态由当前运行实例的真实执行态(而非 `POST /run` 请求往返时长)驱动,与日志 Tab 圆点同源;子 Tab 挂载或页面刷新后,前端通过「最近活跃实例」查询端点续接长耗时运行的「停止」入口与日志/事件流,避免运行态丢失或误判。
