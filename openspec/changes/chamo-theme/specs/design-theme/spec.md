...
- **THEN** 左栏底色（`--sidebar` ≈0.948）明显深于页面底（≈0.978）与内容卡片（纯白），三层无需边框即可区分
...
`chart-1..5` SHALL 为五个互异色相的「织线」色板：茶、钴蓝、金、翠、紫（浅色取值见 design.md D1），用于分类数据可视化；SHALL NOT 再为绿色单色渐变。
...
CopilotChat 面板（`[data-copilotkit]` 作用域）SHALL 继续经 CSS 变量继承跟随 app 主题：对话面填充对齐 `--sidebar`、文字/边框/primary 随主题 token；自绘滚动条 `.dw-textarea-thumb` SHALL 使用茶墨家族暖灰（亮 `oklch(0.45 0.02 80)` / 暗 `oklch(0.74 0.02 75)`），不残留旧 taupe 取值。
...
Monaco 编辑器与 chat 代码块 SHALL 继续共用 `lib/syntax-palette.ts` 的同一套 `buildSyntaxTheme()` 主题对象；调色板 SHALL 从茶墨主题派生：keyword=茶墨家族、func=钴蓝、comment/operator=暖灰阶（完整角色表见 design.md D5），亮/暗各一套。

#### Scenario: 两端高亮一致且锚定茶墨

- **WHEN** 同一段 SQL 分别在 Monaco 编辑器与 chat 代码块中渲染
- **THEN** 两端 token 颜色一致，keyword 呈茶墨家族色而非旧 emerald

### Requirement: 设计真相源同步

`frontend/DESIGN.md` 的 YAML tokens 与色彩章节 SHALL 与 `app/globals.css` 实际取值保持一致（含新增 link/success/warning/info），并 SHALL 通过 `pnpm design:lint` 校验；主题维护方式说明 SHALL 更新为「YAML tokens 手工维护，与 globals.css 双向同步」。
