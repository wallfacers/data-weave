...
- [x] 10.1 后端全量编译
- [x] 10.2 启动后端 + 前端，浏览器打开 `http://localhost:4000`
- [x] 10.3 验证任务 CRUD：创建草稿 → 搜索 → 编辑 → 发布上线 → 下线 → 删除
- [x] 10.4 验证实例操作：手动触发工作流 → 暂停 → 恢复 → 终止
- [x] 10.5 验证日志查看：执行任务后查看完整日志（不截断）
- [x] 10.6 验证日期格式：所有时间字段显示为 `yyyy-MM-dd HH:mm:ss`
- [x] 10.7 验证 Agent 集成：通过对话让 Agent 创建/修改/删除任务，确认 MCP 工具可用
