...
- **Java sidecar supervisor**（`dataweave-api` infrastructure 层，基于 `ProcessBuilder`）：按 `os.name`/`os.arch` 解析 `workhorse-agent-{goos}-{goarch}[.exe]`，以 `serve --config <deploy/workhorse/config.yaml> --host 127.0.0.1 --port 8300` 拉起；探 `/health` 收敛；状态机 `Disabled → Probing → {Adopted | Starting → Healthy → Restarting} | Failed`。
