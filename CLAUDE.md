...
| 任务失败智能运维 · 监督席指挥中心（069） | 任务失败→Agent 自动诊断-处置-升级的一等载体 `incident` + 监督席直播流首屏。**巡检开单**：`IncidentSweeper` 只读观察 FAILED/SUSPENDED（对调度内核零侵入，守 SC-008）→ `tryOpen`（`UNIQUE(tenant_id,open_key)` 单赢）/幂等归并（`incident_instance` PK）。**诊断**：`IncidentAgentService.diagnose` CAS 认领→采证（chip 直播）→确定性凭据指纹 `CredentialFingerprint` 前置免 LLM→`DiagnosisPrompt` LLM 分型（TRANSIENT/RESOURCE/CODE/UPSTREAM_DATA/CONFIG_CREDENTIAL/UNKNOWN）。**处置=纯确定性梯度**：`RemediationPlanner`（LLM 只分型，Java 拥有全部处置决策）→ RERUN/RESUME_CHECKPOINT/ADJUST_RESOURCES/PROPOSE_FIX/ESCALATE；`CONFIG_CREDENTIAL`/`UPSTREAM_DATA` 恒 ESCALATE（零 rerun）、超 `maxAutoActions` 短路 ESCALATE。**CODE 修复**：LLM 全量新内容→`incident_proposal`(PENDING)→闸门 `incident_publish_fix`(L3 人审)→发布(基线陈旧校验)+重跑验证；验证失败→回滚基线(新快照，版本只进不退)+转人工（防循环：`actOrVerify` 优先 PUBLISHED 提案分支，绝不再生成第二份）。**人工协同**：`markHandled`/`reverify`（NEEDS_HUMAN→ACTING 经 `close_kind` 提示位判 AUTO/HUMAN_ASSISTED 收口，`escalate` 清位对偶）/`closeManual`。**监督席直播**（US4）：SSE `GET /api/incidents/stream`（snapshot 先发→桥接 `dw:incident:evt:{projectId}`，Last-Event-ID 补齐持久化消息、瞬态不重放）；`IncidentConversationService`（`POST /{id}/chat` 流式 delta+结构化动作白名单过闸门）；`IncidentBriefingService`（脏集防抖重生成，**数字永远 SQL 实时算 SC-010**，LLM 缺席退确定性模板）。前端首屏 `supervision-view`（`lib/supervision/` store 归约 + `use-incident-stream` SSE + `briefing-banner`/`live-feed`/`incident-thread`，智能感=思考呼吸点/工具 chip 点亮/delta 打字流，全 motion-safe）。MCP `query_incidents`/`incident_reverify`；schema `0.19.0`（+incident/incident_message/incident_proposal/incident_briefing 4 表 + task_def(_version).resources_json + lineage_agent_config.ops_enabled）；`specs/069-agent-incident-ops/` |
| 列级血缘 · 联合表+列重训（067） | 把 065 显式 defer 的列级血缘从空壳（gold/评测/serving 全表级）推进到**模型级·可评测·诚实可发表**，**不新增人工标注**（列 gold 恒 teacher 共识银标=循环性）。**探明空壳只在两个卡点**：`build_gold_b:75`/`build_silver:96` 造 gold/银标时把列硬抹 `columns:None`，但 teacher（`clients.py` SYSTEM_PROMPT）**早已免费吐列** → 列级边际成本≈普通表级重标。**门①表级正交（硬）**：`metrics.py` 加**独立列打分块**（独立 key `col_tp/fp/fn/halluc`），表级 8 key 逐字节不变=断言单测数学钉死；三态列语义 `null`/`[]`/`*`→弃权（`canon_col` 剥表限定前缀）。**门②表级曲线保护（硬）**：`run-col-*` 是列增强**新家族**，既有已发布 `weft-lineage-extractor-*` 只读不覆盖；相对判据=vs 既有 3B 同集 McNemar 不显著退化。**★真跑核心发现**：首训暴露**真·表列权衡**（加列致表 r 0.845→0.649），**隔离消融**（同 979 剥列训纯表，唯一变量=列）干净分解 −0.047 语料规模+**−0.149 加列监督**（截断假设先证伪）；**缓解重训**（LoRA r16→32/epochs2→3 直击容量争用）翻成正结果=交付 `run-col-3b-mit` 表 f1 0.763≈纯表天花板 0.769（权衡几近消除）、列 p0.803/r**0.840**/f10.821（全过 SC）、门②干净 PASS（vs published McNemar p=0.754）。列 scale 非干净单调（分母混淆，诚实报中性）；SQLLineage 列 r 0.143 vs 模型 0.839=招牌图延伸列级。成本 ≈¥25（SC-007 4× 裕度，从 label 真实 usage 算）。证据独立 `out/PAPER-EVIDENCE-067.md`（不碰 065）；`sft_qlora.py` 加 `--lora-r/--lora-alpha`；语料/权重/preds/gold gitignored 走 HF。`specs/067-column-lineage/` |
