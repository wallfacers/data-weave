...
| 三厂商共识 gold · 列级 frontier（068） | 引 **GPT-5.6（OpenAI 中转站，跨 qwen/deepseek 的第三独立厂商）** 作 teacher 破 067 gold 循环可信度 + 探列级两全。**★GPT 用 httpx 裸 POST**（`llm/clients.py`，OpenAI SDK 的 `x-stainless-*` 头触发中转站 WAF `PermissionDenied`）；`m_gpt`=sol(gold)/`m_gpt_bulk`=luna(silver)。共识 `build_gold_b`(2-of-3 主尺+3-of-3 高置信)/`build_silver`(2-of-3 多数)。**US1 破循环达成**：GPT vs 067 gold 一致率表 0.976/列 0.958（`agreement_report`）+ held-out 泛化（`run-col-3b-mit` 未见 GPT 在其独立确认边表召回 0.782，`heldout_vendor_eval`）。**★★核心科学发现=列级 frontier 尖锐双区、3B/LoRA 表与列不可兼得**：密度扫描（`data/thin_columns.py` 保表边只稀释列）证列强区（密度≥45%:列F1 0.88-0.93/表R卡0.65）⟷表强区（密度≤31%:表R0.776/列F1崩0.58），膝点 31-45% 陡降，中间被两端专家碾压；根因=列监督负载（非纯语料，col70@45%单点曾误判被 col50@31%证伪）；容量 r32→r64 只沿 frontier 微移。**两全需 7B（12G 装不下 defer）**。门②表精度 parity 守住（`run-tri-3b-mit` vs 059 McNemar p=1.000）。交付=**frontier 表征 + 两专家点**（列专家 `run-tri-3b` C0.93/T0.65 · 表专家 `run-tri-3b-col50` T0.78/C0.58 严格优于 067-mit）非单一两全模型。限制②治理路由缓解（`governance_routing` 3-of-3 auto 70%/分歧 review 30%，接 063 信封）；限制①动态名范围外。成本真实后台账单 **$2.42≈¥17**（勿自估单价，曾误估¥116被证伪低~7×）。证据独立 `out/PAPER-EVIDENCE-068.md`（不碰 065/067）。`specs/068-tri-vendor-gold/` |
...
at specs/066-remove-alert-incident/plan.md
<!-- SPECKIT END -->
