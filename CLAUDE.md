...
This repo is routinely written by **multiple agents at the same time** — in separate worktrees/branches, and sometimes the same branch. Work you did not author is **load-bearing until proven otherwise**.
- **Never discard, revert, roll back, or overwrite another agent's changes or commits** without explicit user authorization. Forbidden unless authorized: `git reset --hard`, `git checkout -- <path>`, `git restore`, `git clean -fd` over work you didn't create; `git revert` / `git push --force` over commits you didn't make; deleting files, dropping hunks, or rewriting sections because they "look wrong" or clash with your plan; stashing/squashing away someone else's in-flight edits. If unfamiliar code blocks your change, **route around it or ask — don't delete it.**
- **Before you touch a file, detect others' work**: `git status`, `git log --oneline -15`, `git diff`, and read the file fresh — never trust a stale in-context copy. If you can't tell whether something is yours or another agent's, assume it's another agent's.
- **On collision, STOP and escalate — mandatory.** When another agent's edits conflict with yours (same file/region, contradictory intent), duplicate/overlap your work, or leave the intent genuinely ambiguous, **do not** resolve it by picking a winner, merging blindly, or deleting a side. **Halt, surface the exact conflict** (files + line ranges + the two competing intents, with `git log`/`git diff` evidence) **and ask the user to adjudicate.** Silent overwrite is the one action forbidden even under deadline pressure; escalating is always cheaper than destroying a teammate's finished work.
