...
-- Schema Version: 0.22.0（= 项目发布版本，严格 SemVer；改结构必升版本，见 docs/architecture.md）
...
--     → 071 虚拟管家监督席=0.21.0（+patrol_routine/patrol_run/patrol_report/companion_message 4 表：
--       巡检例程/执行历史/项目级共享汇报/管家会话；+patrol_run UNIQUE(routine_id,scheduled_fire_time) 幂等；seed 四领域默认例程）。
--     → 删除 incident 监督席=0.22.0（删除 incident/incident_instance/incident_message/incident_proposal/incident_briefing 5 表 + incident_* policy_rule 种子；保留虚拟管家 companion 域）。
...
-- incident 域已于 0.22.0 删除：下列 DROP 清理旧库（≤0.21.0）升级残留，全新库 no-op。
...
-- 071 虚拟管家监督席：4 张 companion 域表（与下方 CREATE 配对，保 mode=always 与共享内存库幂等）。
DROP TABLE IF EXISTS companion_message;
DROP TABLE IF EXISTS patrol_report;
DROP TABLE IF EXISTS patrol_run;
DROP TABLE IF EXISTS patrol_routine;
...
INSERT INTO schema_version (version, applied_at, description)
VALUES ('0.21.0', CURRENT_TIMESTAMP, '071 虚拟管家监督席：+patrol_routine/patrol_run/patrol_report/companion_message 4 表（巡检例程/执行历史/项目级共享汇报/管家会话）；patrol_run UNIQUE(routine_id,scheduled_fire_time) 幂等防重；seed 四领域默认例程');
INSERT INTO schema_version (version, applied_at, description)
VALUES ('0.22.0', CURRENT_TIMESTAMP, '删除 incident 监督席：删除 incident/incident_instance/incident_message/incident_proposal/incident_briefing 5 表 + incident_* policy_rule 种子（保留虚拟管家 companion 域）');
...
-- ============================================================
-- 域 · 虚拟管家监督席（companion：071）
-- 巡检例程(routine) → 执行历史(run) → 项目级共享汇报(report) + 管家会话(message)
-- 巡检由平台调度内核模式触发（UNIQUE guard 单赢创建 + SKIP LOCKED claim + CAS 推进），brain 外呼在事务外。
-- ============================================================

-- 巡检例程：某领域的计划巡检定义（领域/启停/cron/范围/超时），平台可治理任务（US4）。
-- 项目内按领域唯一；enabled=0 的例程不参与巡检（FR-006 单独启停）。seed 四领域默认例程见 data.sql。
CREATE TABLE patrol_routine (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id       BIGINT NOT NULL,
    project_id      BIGINT NOT NULL,
    domain          VARCHAR(32) NOT NULL,            -- TASK_FAILURE | MACHINE | DATA_QUALITY | CODE_QUALITY
    enabled         SMALLINT NOT NULL DEFAULT 1,     -- 0=停用 1=启用（FR-006）
    cron_expression VARCHAR(64) NOT NULL,            -- Spring CronExpression 6 字段（含秒，无 ?）
    scope_json      TEXT,                            -- 领域范围参数（目录/标签过滤等），JSON
    timeout_seconds INTEGER NOT NULL DEFAULT 120,    -- 单轮超时（R7），超时即产“未完成”汇报
    created_by      BIGINT,
    updated_by      BIGINT,
    created_at      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted         SMALLINT NOT NULL DEFAULT 0,
    version         INTEGER NOT NULL DEFAULT 0,      -- 乐观锁（US4 PATCH 治理）
...
CREATE INDEX idx_patrol_routine_tenant_project ON patrol_routine (tenant_id, project_id);
CREATE INDEX idx_patrol_routine_enabled ON patrol_routine (enabled, deleted);

-- 巡检执行历史（US4 可追溯）：每轮 run 一行，UNIQUE(routine_id, scheduled_fire_time) 防重复触发（抄 cron_fire 幂等地基）。
-- 状态机：CLAIMED → RUNNING → {SUCCEEDED | FAILED | TIMEOUT}，一律 CAS 推进（调度不变量②，禁回退）。
CREATE TABLE patrol_run (
    ...
    scheduled_fire_time  TIMESTAMP NOT NULL,         -- 计划触发时刻（幂等键的一部分）
    state                VARCHAR(16) NOT NULL DEFAULT 'CLAIMED',  -- CLAIMED → RUNNING → SUCCEEDED/FAILED/TIMEOUT
    started_at           TIMESTAMP,
    finished_at          TIMESTAMP,
    summary              TEXT,                        -- 本轮结论摘要
    error                TEXT,                        -- 失败原因（产“未完成”汇报时同源）
    created_at           TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at           TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    version              INTEGER NOT NULL DEFAULT 0, -- CAS 乐观锁
...
CREATE INDEX idx_patrol_run_tenant_project ON patrol_run (tenant_id, project_id);
CREATE INDEX idx_patrol_run_claim ON patrol_run (state, scheduled_fire_time);
CREATE INDEX idx_patrol_run_routine ON patrol_run (routine_id, created_at DESC);

-- 巡检汇报（项目级共享）：一轮巡检的结构化产出。关闭为项目级共享（一人关闭全员消失，clarify 决议）。
-- severity=INFO 含“未完成”汇报；关联对象已消失时由 detail_json 快照名兜底展示（边界用例）。
CREATE TABLE patrol_report (
    id              BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id       BIGINT NOT NULL,
    project_id      BIGINT NOT NULL,
    run_id          BIGINT,                          -- 产出来源（执行历史↔汇报关联，US4-AS2）；允许 NULL 兜底
    domain          VARCHAR(32) NOT NULL,            -- 冗余自 routine，查询友好
    severity        VARCHAR(16) NOT NULL,            -- DANGER | WARN | OK | INFO（INFO 含“未完成”）
    title           VARCHAR(255) NOT NULL,
    summary         TEXT,                            -- 摘要（管家播报文案来源）
    detail_json     TEXT,                            -- 结构化明细：关联对象(type+id+name)/聚合计数/建议动作
    aggregate_count INTEGER NOT NULL DEFAULT 1,      -- 同领域聚合窗口内异常条数（FR-011）
    status          VARCHAR(16) NOT NULL DEFAULT 'UNREAD',  -- UNREAD → READ → CLOSED（项目级共享，CLOSED 终态）
    closed_by       VARCHAR(128),                    -- 关闭人显示名（项目级共享处置留痕）
    closed_at       TIMESTAMP,
    created_at      TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_patrol_report_stack ON patrol_report (project_id, status, created_at DESC);
CREATE INDEX idx_patrol_report_run ON patrol_report (run_id);

-- 管家会话消息：全局会话（report_id=NULL）或锚定某汇报的上下文会话（FR-013）。
-- 发言者身份服务端认定（actor/actor_name，沿 070 监督席标准）。
CREATE TABLE companion_message (
    id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tenant_id        BIGINT NOT NULL,
    project_id       BIGINT NOT NULL,
    report_id        BIGINT,                         -- NULL=全局会话；非 NULL=锚定该汇报的上下文会话（FR-013）
    role             VARCHAR(16) NOT NULL,           -- USER | AGENT | SYSTEM
    actor            VARCHAR(64),                    -- 用户名(服务端认定) | companion-agent | system
    actor_name       VARCHAR(128),                   -- 发言者显示名（070 身份标准延续）
    content          TEXT,                           -- Markdown 正文
    brain_session_id VARCHAR(128),                   -- 对应 workhorse session（排障/续聊）
    created_at       TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_companion_message_role CHECK (role IN ('USER','AGENT','SYSTEM'))
);
CREATE INDEX idx_companion_message_session ON companion_message (project_id, report_id, created_at);
