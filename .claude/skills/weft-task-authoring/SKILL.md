...
dw context <taskId>        # 2. 【编辑前接地】取真实读写表/上下游/列血缘/三态存在性
[写/改任务文件]              # 3. 创作任务定义 + 脚本体（依接地事实，勿凭空臆测表名/上游）
dw run <task>              # 4. 本机真跑验证
dw diff                    # 5. 差异预览（对服务器零写入）
dw push [--force]          # 6. 推回服务器（幂等覆盖 + 版本快照）
dw run --test <task>       # 7. TEST 模式提交服务器验证
```
...
改一个已有任务、或要复用/衔接上游产物前，**先取血缘接地事实**，避免凭空臆测表名、虚构上游、写错列：

```
dw context <taskId> [--depth N] [--json]   # 读写表 → 上下游表邻居 + 表/列血缘 + 三态接地
dw deps <taskId> [--json]                  # 依赖视图：声明 DAG + 推导血缘合并，带 origin
```

- **三态接地**：`PRESENT`=数据源确有此表；`UNGROUNDED`=不存在（别当上游）；`INFERRED`=未绑数据源无法判定。未接地的表**不虚构**其上游。
- **依赖 origin**：`DECLARED`=仅工作流 DAG 声明；`DERIVED`=仅血缘推导；`BOTH`=一致。`DECLARED` 独有=可能僵依赖，`DERIVED` 独有=可能漏声明——据此对齐声明与实际。
- 纯读、确定性、零 LLM、零持久化；`--json` 供程序化消费。同一事实经 MCP `query_authoring_context`/`query_task_deps` 亦可得（语义一致）。
