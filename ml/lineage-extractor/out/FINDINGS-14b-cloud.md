...
### 14B-lw3 出数(2026-07-18 凌晨)——历史最佳单模型,距双门一线

| 模型 | 表 P | 表 R | 表 F1 | 列 F1 | 方向 | 双门 |
|---|---|---|---|---|---|---|
| 14B tri plain | 0.880 | 0.679 | 0.766 | 0.921 | 0.677 | ✗/✓ |
| **14B tri lw3** | 0.887 | **0.727** | **0.799** | **0.856** | **0.727** | ✗(差0.023)/✓ |

- lw3 在 14B 复现 068 的 3B 规律且幅度相当:表 F1 +0.033(0.766→0.799),代价列 F1 −0.065(0.921→0.856)+方向 +0.05。
- **表 F1 0.799 = 全项目历史最佳单模型**(超 3B lw3 0.781,且列 0.856>0.825 双优)。
- 严格双门:列门过(0.856≥0.85),**表召回门差 0.023**(0.727 vs 0.75,n=129 上≈3 张表)。接续 14B-lw4 再压一档冲门(列富余仅 0.006,有跌破列门风险,如实记录)。
- 事故记录:lw3→lw4 间隙我误跑 `shutdown -c`(想取消定时关机)导致容器重启;14B-lw3 产物完好(数据盘),7B-lw3 训练 68% 被杀重跑。**教训:AutoDL 容器内禁碰电源类命令**。

### 14B-lw4 出数(2026-07-18)——loss 权重扫完,严格双门仍不可达(负结果,确证 frontier 硬约束)

| 14B 档 | 表 P | 表 R | 表 F1 | 列 P | 列 R | 列 F1 | 方向 | 双门(表R≥0.75 且 列F1≥0.85) |
|---|---|---|---|---|---|---|---|---|
| plain(W1) | 0.880 | 0.679 | 0.766 | 0.923 | 0.919 | 0.921 | 0.677 | 列✓ 表✗(差0.071) |
| lw3(W3) | 0.887 | 0.727 | 0.799 | — | — | 0.856 | 0.727 | 列✓ 表✗(差0.023) |
| **lw4(W4)** | 0.896 | **0.761** | **0.823** | 0.925 | 0.660 | 0.771 | 0.759 | 表✓ 列✗(差0.079) |

- **lw4 把表召回顶过门**(0.761≥0.75,表 F1 0.823=历史最高),**但列 F1 崩到 0.771**(跌破 0.85 门 0.079)。W1→W3→W4 三点画出干净单调 pareto 前沿:表 R 0.679→0.727→0.761,列 F1 0.921→0.856→0.771,**每换 1pt 表 R 约赔 2.5pt 列 F1**。
- **核心负结果(确证)**:14B 单模型即使扫 loss 权重,**pareto 前沿不穿过双门矩形**——lw3/lw4 分别只过一门,几何上 lw3 离双门角最近(表差 0.023、列余 0.006),但斜率 2.5 意味着补那 0.023 表 R 要赔 ~0.058 列 F1,远超余量。**068 在 3B 上的「表列竞争不可兼得」结论在 14B 上复现:放大让前沿整体外移(14B lw3 0.799/0.856 双优于 3B lw3 0.781/0.825),但 frontier 本身不消失,严格双门仍需非单模型手段(068 US6 双专家融合)。**
- **最佳均衡单模型 = 14B-lw3**(表 F1 0.799 + 列 F1 0.856,离双门角最近且双高);**最佳表专家 = 14B-lw4**(表 F1 0.823/表 R 0.761,过表门);**最佳列专家 = 14B-plain**(列 F1 0.921)。这三点正好是融合的理想输入(表专家 lw4 定表集 + 列专家 plain 嫁接列)。

## ★★★ 全扫描总表 + 7B loss 加权失效(2026-07-18 收口)

tri 真实银标训练 → 真实 tri gold 评测(非空 129),全部规模×loss权重扫描:

| 模型 | 表 P | 表 R | 表 F1 | 列 F1 | 方向 | 双门(表R≥0.75 且 列F1≥0.85) |
|---|---|---|---|---|---|---|
| 3B col-spec(068) | 0.822 | 0.645 | 0.723 | 0.931 | — | 列✓ 表✗ |
| 3B lw3(068 最佳) | — | — | 0.781 | 0.825 | — | 双差角 |
| 7B plain | 0.813 | 0.624 | 0.706 | 0.924 | 0.619 | 列✓ 表✗ |
| 7B lw3 | 0.809 | 0.628 | 0.707 | 0.894 | 0.626 | 列✓ 表✗ |
| 14B plain | 0.879 | 0.679 | 0.766 | 0.921 | 0.677 | 列✓ 表✗ |
| **14B lw3** | 0.887 | 0.727 | **0.799** | **0.856** | 0.727 | 列✓ 表✗(差0.023,最接近) |
| 14B lw4 | 0.896 | **0.761** | **0.823** | 0.771 | 0.759 | 表✓ 列✗(差0.079) |

**三条终局结论**:

1. **规模有效但非线性**:表 F1 3B col-spec 0.723 → 7B 0.706(≈持平,甚至微降)→ 14B 0.766;方向 3B→7B 0.62→14B 0.68→lw4 0.76。**7B 几乎 = 3B 列专家,14B 才显现规模红利**——中间档 7B 不是划算的规模点(印证 068「7B 为出路」应读作「14B 才是出路」)。

2. **loss 加权杠杆有容量门槛(新负结果)**:同样 W3,**14B 表 R +0.048**(0.679→0.727,列 F1 −0.065)显著奏效;**7B 表 R 仅 +0.004**(0.624→0.628,列 F1 还降 0.030)几乎无效。7B 已在自身容量墙上,表结构 loss 加权挤不出表召回——**loss 加权是「够大才用得动」的杠杆**,进一步实证容量是主约束。

3. **严格双门在所有单模型上不可达,即使 14B 全 loss 权重扫描(核心负结果确证)**:14B 的 W1/W3/W4 画出干净单调 pareto(表 R 0.679→0.727→0.761 换列 F1 0.921→0.856→0.771),前沿整体随规模外移(14B lw3 双优于 3B lw3),**但不穿过双门矩形**。最接近的 14B-lw3 差表 R 0.023,斜率 2.5 使补齐必赔穿列门。**068 在 3B 上的「表列竞争不可兼得」结论在 14B 严格复现——放大缓解不消除 frontier,严格双门的工程解仍是 068 US6 双专家融合(表专家 14B-lw4 定表集 + 列专家 14B-plain 嫁接列)。**
