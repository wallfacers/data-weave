...
| **[脊椎] 逐规模真实泛化单调升**（同评测集 C′：0.5→1.5→3B）| `out/significance-c.md` | f1 **0.416→0.563→0.785**；p 0.450→0.543→0.743；r 0.387→0.586→0.832（n=104）| ✅ 真跑 |
| **[脊椎] 记忆↓ 与 泛化↑ 反向耦合**（规模越大逐字记忆越低却真实泛化越高）| `out/leak-curve.md` × `out/significance-c.md` | 记忆 37→22→11% **反向** f1 0.416→0.563→0.785 | ✅ 真跑（交叉锚定）|
...
| **[支撑·动机] 工具在脚本子集 recall≈0，模型救回（且救回幅度随规模单调升）** | `out/baselines-c.md` | SQLLineage@script r **0.029**[0,0.121]；模型 Δr vs 工具 0.5B **+0.206**→1.5B **+0.353**→3B **+0.559**，三档全显著 | ✅ 真跑 |
...
| **[稳健性·附录] gold C′ 空侧干净**（deepseek 复标争议空脚本零翻标）| `out/arbitrate-gold.md` | 争议空脚本 1 → pro 复标 **0 翻标**（模型 FP 属实）= min-agree=2 共识空侧无 label noise | ✅ 真跑（验证性）|
| **[稳健性·附录] auto-gold 扩容 SC-006（混合，非头条）** | `out/expand-gold.md` / `out/significance-c-expanded.md` | 非空 104→114（+10，`robustness_only`）；CI **字面达标但边际混合**：model-3b/teacher-m1 precision 收窄，recall/f1 反略变宽 → 印证 US4"默认砍" | ✅ 真跑（诚实混合）|
